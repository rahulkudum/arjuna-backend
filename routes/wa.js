...
async function sendTo(phoneOrContact, message) {
 ...
 try {
  ...
  await page.waitForSelector("#main > footer > div.vR1LG._3wXwX.copyable-area > div._2A8P4 > div > div._2_1wd.copyable-text.selectable-text", {
   timeout: 60000,
  });
  ...
  counter.success++;
  return "success" + phone + "-" + phoneOrContact.name;
 } catch (err) {
  process.stdout.clearLine();
  process.stdout.cursorTo(0);
  process.stdout.write(`${phone} Failed\n`);
  counter.fails++;
  return "fail" + phone + "-" + phoneOrContact.name;
 }
}
...
async function end() {
 ...
 return `Result: ${counter.success} sent, ${counter.fails} not sent`;
}
